...
int DRUnit::set_parameters(int in_aid1, int in_aid2,
			   real in_coef_low, real in_coef_high,
			   real in_dist_low, real in_dist_high){
  atomid1 = in_aid1;
  atomid2 = in_aid2;
...
int DistRestraintObject::add_drunit(int in_aid1, int in_aid2,
				   real in_coef_low, real in_coef_high,
				   real in_dist_low, real in_dist_high){
  drunits[n_drunits].set_parameters(in_aid1, in_aid2,
				    in_coef_low, in_coef_high,
				    in_dist_low, in_dist_high);
  n_drunits++;
  return n_drunits;
}
real_fc DistRestraintObject::apply_restraint(int n_atoms, real** crd, PBC& pbc, 
					 real** force){
...
real_fc DistRestraintHarmonic::apply_restraint(int n_atoms,
  ...
  for( int i = 0; i < n_drunits; i++){
    real diff[3];
    pbc.diff_crd_minim_image(diff, crd[drunits[i].get_atomid1()],
			     crd[drunits[i].get_atomid2()]);
