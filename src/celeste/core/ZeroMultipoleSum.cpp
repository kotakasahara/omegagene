...
ZeroMultipoleSum::ZeroMultipoleSum(const int           in_zms_mode,
                                   const real          in_alpha,
...
int ZeroMultipoleSum::cal_self_energy(const int &  n_atoms,
    ...
    for (int i = 0; i < n_atoms; i++) {
        energy_self[i] = 0.0;
        energy_self[i] =
            (-piewald * charge[i] * charge[i] - zmcore * charge[i] * (charge[i] + dwork[i])) * 0.5 * CHARGE_COEFF;
...
int ZeroMultipoleSum::set_zms_params() {
    ...
    if (ewald_alpha < EPS) {
        ...
        coeffd4 =
            (errorfc + expterm * (tmp + 2.0 / 3.0 * tmp_3 + 1.0 / 6.0 * tmp_5 + 1.0 / 3.0 + tmp_7) * 2.0 / root_pi)
            * 24.0 / cutoff_5;
    }
    ...
    zhcoeff2 = -35.0 / 32.0 * coeffd1 / cutoff - 19.0 / 32.0 * coeffd2 - 1.0 / 96.0 * coeffd4 * cutoff_2
               - 1.0 / 8.0 * coeffd3 * cutoff;
    zhcoeff4 = 1.0 / 64.0 * coeffd4 + 35.0 / 64.0 * coeffd1 / cutoff_3 + 35.0 / 64.0 * coeffd2 / cutoff_2
    ...
    zhcore = errorfc / cutoff - zhcoeff2 * cutoff_2 - zhcoeff4 * cutoff_4 - zhcoeff6 * cutoff_6
             - zhcoeff8 * cutoff_6 * cutoff_2;

    return 0;
}
int ZeroMultipoleSum::calc_zero02pole_excess_alpha0(real_pw &      ene_ele,
                                                    real_pw &      grad_coeff,
                                                    const real_pw &r12,
                                                    const real_pw &r12_2,
...
int ZeroMultipoleSum::calc_zero02pole_excess(real_pw &      ene_ele,
                                             real_pw &      grad_coeff,
                                             const real_pw &r12,
                                             const real_pw &r12_2,
                                             const real_pw &r12_inv,
...
int ZeroMultipoleSum::calc_zero04pole_excess_alpha0(real_pw &      ene_ele,
                                                    real_pw &      grad_coeff,
                                                    const real_pw &r12,
                                                    const real_pw &r12_2,
                                                    const real_pw &r12_inv,
...
int ZeroMultipoleSum::calc_zero04pole_excess(real_pw &      ene_ele,
                                             real_pw &      grad_coeff,
                                             const real_pw &r12,
                                             const real_pw &r12_2,
                                             const real_pw &r12_inv,
    ...
    grad_coeff =
        -cc * (errorfc * r12_3_inv + piewald * exp(-tmp * tmp) * r12_2_inv + zqcoeff22 + zqcoeff44 * r12_2 - r12_3_inv);
    return 0.0;
}
int ZeroMultipoleSum::calc_zero08pole_excess_alpha0(real_pw &      ene_ele,
                                                    real_pw &      grad_coeff,
                                                    const real_pw &r12,
                                                    const real_pw &r12_2,
                                                    const real_pw &r12_inv,
...
int ZeroMultipoleSum::calc_zero08pole_excess(real_pw &      ene_ele,
                                             real_pw &      grad_coeff,
                                             const real_pw &r12,
                                             const real_pw &r12_2,
                                             const real_pw &r12_inv,
    ...
    grad_coeff = -cc * (errorfc * r12_3_inv + piewald * exp(-tmp * tmp) * r12_2_inv + zocoeff22 + zocoeff44 * r12_2
                        + zocoeff66 * r12_4 - r12_3_inv);
    return 0.0;
}
int ZeroMultipoleSum::calc_zero16pole_excess_alpha0(real_pw &      ene_ele,
                                                    real_pw &      grad_coeff,
                                                    const real_pw &r12,
                                                    const real_pw &r12_2,
                                                    const real_pw &r12_inv,
...
int ZeroMultipoleSum::calc_zero16pole_excess(real_pw &      ene_ele,
                                             real_pw &      grad_coeff,
                                             const real_pw &r12,
                                             const real_pw &r12_2,
                                             const real_pw &r12_inv,
    ...
    grad_coeff = -cc * (errorfc * r12_3_inv + piewald * exp(-tmp * tmp) * r12_2_inv + zhcoeff22 + zhcoeff44 * r12_2
                        + zhcoeff66 * r12_4 + zhcoeff88 * r12_6 - r12_3_inv);
    return 0.0;
}
int ZeroMultipoleSum::calc_zero02pole_alpha0(real_pw &      ene_ele,
                                             real_pw &      grad_coeff,
                                             const real_pw &r12,
                                             const real_pw &r12_2,
                                             const real_pw &r12_inv,
...
int ZeroMultipoleSum::calc_zero02pole(real_pw &      ene_ele,
                                      real_pw &      grad_coeff,
                                      const real_pw &r12,
                                      const real_pw &r12_2,
                                      const real_pw &r12_inv,
...
int ZeroMultipoleSum::calc_zero04pole_alpha0(real_pw &      ene_ele,
                                             real_pw &      grad_coeff,
                                             const real_pw &r12,
                                             const real_pw &r12_2,
                                             const real_pw &r12_inv,
...
int ZeroMultipoleSum::calc_zero04pole(real_pw &      ene_ele,
                                      real_pw &      grad_coeff,
                                      const real_pw &r12,
                                      const real_pw &r12_2,
                                      const real_pw &r12_inv,
...
int ZeroMultipoleSum::calc_zero08pole_alpha0(real_pw &      ene_ele,
                                             real_pw &      grad_coeff,
                                             const real_pw &r12,
                                             const real_pw &r12_2,
                                             const real_pw &r12_inv,
...
int ZeroMultipoleSum::calc_zero08pole(real_pw &      ene_ele,
                                      real_pw &      grad_coeff,
                                      const real_pw &r12,
                                      const real_pw &r12_2,
                                      const real_pw &r12_inv,
...
int ZeroMultipoleSum::calc_zero16pole_alpha0(real_pw &      ene_ele,
                                             real_pw &      grad_coeff,
                                             const real_pw &r12,
                                             const real_pw &r12_2,
                                             const real_pw &r12_inv,
...
int ZeroMultipoleSum::calc_zero16pole(real_pw &      ene_ele,
                                      real_pw &      grad_coeff,
                                      const real_pw &r12,
                                      const real_pw &r12_2,
                                      const real_pw &r12_inv,
    ...
    const real_pw errorfc = erfc(tmp);
    ene_ele =
        cc * (r12_inv * errorfc - zhcore - zhcoeff2 * r12_2 - zhcoeff4 * r12_4 - zhcoeff6 * r12_6 - zhcoeff8 * r12_8);
    grad_coeff = -cc * (r12_3_inv * errorfc + piewald * exp(-tmp * tmp) * r12_2_inv + zhcoeff22 + zhcoeff44 * r12_2
...
int ZeroMultipoleSum::calc_debye_huckel(real_pw &      ene_ele,
  ...
  ene_ele = cc * r12_inv * dielectric_inv  * r12_ld_exp;
  grad_coeff = - ene_ele * (r12_inv + debye_length_inv);
  return 0;
}
int ZeroMultipoleSum::calc_null(real_pw &      ene_ele,
				real_pw &      grad_coeff,
				const real_pw &r12,
				const real_pw &r12_2,
				const real_pw &r12_inv,
