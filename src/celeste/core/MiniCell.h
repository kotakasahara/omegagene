...
class MiniCell : public CelesteObject {
  ...
  public:
    ...
    bool check_valid_pair(const int cell1_id, const int cell2_id, const bool cell1_odd, const bool cell2_odd);
    ...
    int get_n_atoms_in_cell(const int cid) {
        ...
        if (cell_crd[cid][2] < n_cells_z[col])
            return N_ATOM_CELL;
        else
            return n_atoms_col[col] % N_ATOM_CELL;
    }
    ...
    int add_work(const int atomid_grid, const real_fc in_w1, const real_fc in_w2, const real_fc in_w3);
    ...
    int print_box_info();
    int set_crds_to_homebox(real *in_crd, int *in_atomids, int in_n_atoms_box);
    int set_max_n_atoms_region();
    int get_region_id_from_crd(int width, int rx, int ry, int rz);
    int get_region_crd_from_id(int width, int regid, int &rx, int &ry, int &rz);
