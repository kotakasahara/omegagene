...
__device__ __inline__ double shfl_xor(double value, int const lane, int const warpsize) {
    return __hiloint2double(__shfl_xor(__double2hiint(value), lane, warpsize), __shfl_xor(__double2loint(value), lane, warpsize));
}
...
extern "C" int cuda_alloc_atom_info(int in_max_n_atoms_exbox,
                                    ...
                                    int in_max_n_cells, int in_max_n_cell_pairs, int in_n_columns) {
...
extern "C" int cuda_memcpy_htod_atomids(int *&h_atomids, int *&h_idx_xy_head_cell) {
    ...
    HANDLE_ERROR(cudaMemcpy(d_idx_xy_head_cell, h_idx_xy_head_cell, (n_columns + 1) * sizeof(int), cudaMemcpyHostToDevice));
    return 0;
}
...
extern "C" int cuda_set_cell_constant(const int in_n_cells, const int in_n_atoms_exbox, const int in_n_atom_array,
                                      const int *in_n_cells_xyz, const int in_n_columns, const real_pw *in_l_cell_xyz,
...
extern "C" int cuda_alloc_set_lj_params(real_pw *h_lj_6term, real_pw *h_lj_12term, int n_lj_types, int *h_nb15off,
...
extern "C" int cuda_hostalloc_atom_info(real_pw *&h_crd, int *&h_atomids, real_fc *&h_work, real_fc *&h_energy, int in_max_n_atom_array) {
    max_n_atom_array = in_max_n_atom_array;
...
extern "C" int cuda_hostalloc_cellpair_info(CellPair *&h_cell_pairs, int *&h_idx_head_cell_pairs, int *&h_n_cells_z, int max_n_cell_pairs,
                                            int max_n_cells, int n_columns) {
...
__global__ void kernel_set_nb15off(const int *d_atomids, const int *d_atomids_rev, const int *d_nb15off_orig, int *d_nb15off) {
...
__global__ void kernel_set_atominfo(const int *d_atomids, const int *d_atomtype_orig, const real_pw *d_charge_orig, int *d_atomtype,
                                    real4 *d_crd_chg, int *d_atomids_rev) {
...
extern "C" int cuda_set_atominfo() {
    ...
    kernel_set_atominfo<<<blocks1, REORDER_THREADS>>>(d_atomids, d_atomtype_orig, d_charge_orig, d_atomtype, d_crd_chg, d_atomids_rev);
...
__device__ bool check_15off64(const int atom_idx1, const int atom_idx2, const int *bitmask, int &mask_id, int &interact_bit) {
...
__device__ real_pw cal_pair(real_pw &w1, real_pw &w2, real_pw &w3, real_pw &ene_vdw, real_pw &ene_ele, const real4 &crd_chg1,
                            const real4 &crd_chg2, const int &atomtype1, const int &atomtype2, const real_pw *__restrict__ d_lj_6term,
...
__global__ void kernel_pairwise_ljzd(const real4 *d_crd_chg, CellPair *d_cell_pairs, const int *d_idx_head_cell_pairs,
                                     const int *d_atomtype, const real_pw *__restrict__ d_lj_6term, const real_pw *__restrict__ d_lj_12term,
...
__global__ void pack_cellpairs_array(CellPair *d_cell_pairs, const CellPair *d_cell_pairs_buf, const int *d_n_cell_pairs,
    ...
    if (cellpair.cell_id1 < 0 || cellpair.cell_id2 < 0 || cellpair.cell_id1 >= D_N_CELLS || cellpair.cell_id2 >= D_N_CELLS) { return; }
    const int cp_in_cell1 = cp - cellpair.cell_id1 * D_MAX_N_CELL_PAIRS_PER_CELL;
    if (cp_in_cell1 >= d_n_cell_pairs[cellpair.cell_id1]) {
        printf("Error: cp_in_cell1:%d d_n_cell_pairs:%d cp:%d c1:%d head:%d\n", cp_in_cell1, d_n_cell_pairs[cellpair.cell_id1], cp,
               cellpair.cell_id1, d_idx_head_cell_pairs[cellpair.cell_id1]);
        return;
    }
...
extern "C" int cuda_pairwise_ljzd(const bool flg_mod_15mask) {
    ...
    kernel_pairwise_ljzd<<<blocks, PW_THREADS, 0, stream_pair_home>>>(d_crd_chg, d_cell_pairs, d_idx_head_cell_pairs, d_atomtype,
                                                                      d_lj_6term, d_lj_12term, d_energy, d_work);
    // if(flg_mod_15mask){
...
__device__ int get_column_id_from_crd(const int x, const int y) { return y * D_N_CELLS_XYZ[0] + x; }
...
__device__ int set_cell_pair_bitmask(const int cell_id1, const int cell_id2, const int cell1_id_in_block, const int *s_nb15off,
                                     const int n_atom_cell2, int *pair_mask) {
...
__global__ void kernel_enumerate_cell_pair(const real4 *d_crd_chg, const real2 *d_cell_z, const int *d_idx_xy_head_cell,
                                           const int *d_idx_cell_column, const int *d_atomids, const int *d_nb15off_orig,
                                           const int *d_nb15off, int *d_n_cell_pairs, CellPair *d_cell_pairs) {
...
__global__ void set_cell_pairs_nb15off(const int *d_idx_head_cell_pairs, const int *d_nb15off, const int *d_atomids, const int n_cell_pairs,
    ...
    if (cell1_id_in_block >= MAX_N_CELL_BLOCK) {
        printf("The number of cells in each block exceeds the constant MAX_N_CELL_BLOCK: %d / %d\ncell: %d %d - %d cp_block_first:%d "
               "c1_in_first:%d\n",
               cell1_id_in_block, MAX_N_CELL_BLOCK, cp, cell1_id, cell2_id, cp_block_first, d_cell_pairs[cp_block_first].cell_id1);
    }
...
extern "C" int cuda_enumerate_cell_pairs(int *&h_atomids,
    ...
    kernel_enumerate_cell_pair<<<blocks4, REORDER_THREADS, 0, stream1>>>(                   // d_uni2cell_z,
        d_crd_chg, d_cell_z, d_idx_xy_head_cell, d_idx_cell_column, d_atomids, d_nb15off_orig, d_nb15off, d_n_cell_pairs, d_cell_pairs_buf);
    // d_cell_pairs);
    ...
    pack_cellpairs_array<<<blocks3, REORDER_THREADS, 0, stream1>>>(d_cell_pairs, d_cell_pairs_buf, d_n_cell_pairs, d_idx_head_cell_pairs);

    const int blocks5 = (n_cell_pairs + REORDER_THREADS - 1) / REORDER_THREADS;
    set_cell_pairs_nb15off<<<blocks5, REORDER_THREADS, 0, stream1>>>(d_idx_head_cell_pairs, d_nb15off, d_atomids, n_cell_pairs,
                                                                     d_cell_pairs);
...
extern "C" int cuda_memcpy_htod_cell_pairs(CellPair *&h_cell_pairs, int *&h_idx_head_cell_pairs, int n_cell_pairs) {
    ...
    HANDLE_ERROR(cudaMemcpy(d_idx_head_cell_pairs, h_idx_head_cell_pairs, (n_cells + 1) * sizeof(int), cudaMemcpyHostToDevice));
    HANDLE_ERROR(cudaMemcpyToSymbol(D_N_CELL_PAIRS, &n_cell_pairs, sizeof(int)));
