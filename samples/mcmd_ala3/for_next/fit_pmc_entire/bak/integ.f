...
c*********************************************
c standard input:
c     nfil = number of states
c     flg_neighbor: 1 ... include surrounding states
c                   others ... fit for only one state

        read(5,*) nfil, flg_neighbor

        print*,'#  N of files = ',nfil

        read(11,*) sfac
        print*,'# scaling factor for pdf = ',sfac
c********************
c  Input pdf.
        do kk=1,nfil
          print*,'#  v-st No. = ',kk
...
!kk            erng_n(2,kk)=erng(2,i3)
            if(flg_neighbor.eq.1) then 
               erng_n(2,kk)=erng(2,i2)
            else
               erng_n(2,kk)=erng(2,i1)
            endif
            write(*,*) "DEBUG ", kk
            write(*,*) "DEBUG ", erng(2,i1), erng(2,i2), erng(2,i3)
          endif

!kk          if(i1.ne.0 .and. i3.ne.nfil+1) then
          if(kk.ne.1 .and. kk.ne.nfil) then
            if(flg_neighbor.eq.1) then 
               erng_n(1,kk)=erng(1,i1)
               erng_n(2,kk)=erng(2,i3)
            else
               erng_n(1,kk)=erng(1,i2)
               erng_n(2,kk)=erng(2,i2)
            endif
          endif
...
!          if(i3.eq.nfil+1) then
          if(kk.eq.nfil) then
            ...
            i2=nfil-1
            i3=nfil
!kk            erng_n(1,kk)=erng(1,i1)
            if(flg_neighbor.eq.1) then 
               erng_n(1,kk)=erng(1,i2)
            else
               erng_n(1,kk)=erng(1,i3)
            endif
            erng_n(2,kk)=erng(2,i3)
          endif
